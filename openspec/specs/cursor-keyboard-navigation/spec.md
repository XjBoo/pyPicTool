...
系统 SHALL 仅对最近被鼠标左键点击选中的游标（数据点左键点击、Shift+左键新增、或左键点击其 tooltip）响应 Left/Right/Home/End：方向键移动到本系列的上一/下一有效数据点，Home/End 跳转到第一个/最后一个有效数据点。该移动 SHALL 不受游标锁定状态限制。仅经悬停选中的游标 SHALL 不响应这些按键；悬停改变当前选中（金色高亮）游标 SHALL 不改变键盘移动的目标。未发生任何左键点击时，这些按键 SHALL 不产生游标移动。
...
- **THEN** 游标 A 移动（其他未锁定游标照常同步），游标 B 不作为键盘移动的目标
...
键盘移动选中游标后，系统 SHALL 将其他未锁定游标按目标帧值同步到各自系列最近的有效数据点；其他锁定游标 SHALL 保持原位。

#### Scenario: 移动选中游标同步未锁定的其他系列游标

- **WHEN** 两个系列各有一个未锁定游标，用户对选中游标按 Right
- **THEN** 另一系列的游标移动到其最近有效帧的数据点

#### Scenario: 锁定的其他游标不被同步

- **WHEN** 某非选中游标处于锁定状态，用户移动选中的游标
- **THEN** 该锁定游标保持原位
...
- **WHEN** 某游标处于锁定状态且鼠标在图内悬停移动
- **THEN** 该游标保持原位，未锁定的其他游标照常按帧同步
