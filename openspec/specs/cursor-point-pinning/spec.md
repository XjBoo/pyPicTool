...
- **WHEN** 在普通点选择模式下，用户先普通点击蓝色系列的点，再普通点击红色系列的点
- **THEN** 仅红色点保持活动选中标记和 tooltip，蓝色点的活动选中标记和 tooltip 同时消失

#### Scenario: 普通点击不清除钉选

- **WHEN** 在普通点选择模式下，图上已有一个或多个钉选点，用户普通点击另一数据点
- **THEN** 新点成为唯一活动选中，所有已有钉选点及其 tooltip 保持不变

#### Scenario: 再次点击活动点取消选中

- **WHEN** 在普通点选择模式下，用户左键点击某未钉选数据点使其成为活动选中，随后再次左键点击同一点
...
- **WHEN** 在普通点选择模式下，用户对同一面板中的多个数据点依次执行 Shift+左键点击
...
- **WHEN** 在普通点选择模式下，用户普通左键点击一个钉选点
- **THEN** 仅该钉选点的标记和 tooltip 被移除，不在该位置建立活动选中

#### Scenario: 删除当前钉选点

- **WHEN** 在普通点选择模式下，用户选中一个钉选点并触发单点删除
- **THEN** 仅该钉选点的标记和 tooltip 被移除

#### Scenario: 清除全部钉选点

- **WHEN** 在普通点选择模式下，用户触发全部钉选点清除
